...
int main( int argc, char * argv[] )
{
    ...
    */
    ...
    if (p < 2)
    {
        std::cout << "ERROR:  p must be 2 or more.\n\n" << std::endl;
        exit( 1 );
    }

    if (n > MAXDEGPOLY || n < 2)
    {
        std::cout << "ERROR: n must be between 2 and " << MAXDEGPOLY <<  std::endl;
        exit( 1 );
    }
    ...
    {
        std::cout << "ERROR:  p must be a prime number.\n\n" << std::endl;
        exit( 1 );
    }

    if ((print_hex == YES) && (p != 2))
    {
        std::cout << "WARNING: hex print is available only for modulo 2.\n\n" << std::endl;
        print_hex = NO; // print hex only for GF(2)
    }
    /*
                   n
    ...
    */
    ...
    {
        std::cout << "ERROR:  p to the nth power must be smaller than: " << MAXPTON << std::endl;
        exit( 1 );
    }
    ...
    */
    ...
    if (printStatistics || listAllPrimitivePolynomials)
    {
        ...
        std::cout << "Total number of primitive polynomials = " << num_prim_poly << std::endl;
    }
    ...
    */
    ...
    std::ofstream file_hex;

    if(print_hex == YES)
    {
        std::string file_name( "lsfr_coeffs_" + std::to_string(n) + ".txt");
        file_hex.open(file_name);
        file_hex << "*** LSFR coefficients for n=" << n << "\n\n" << std::endl;
    }
    ...
    do {
        ...
        std::cout << "\nNext trial polynomial: " << std::endl;
        write_poly( f, n );
        std::cout << std::endl;
        #endif
        ...
        */
        ...
        if (const_coeff_is_primitive_root( f, n, p ))
        {
            ...
            #ifdef DEBUG_PP_PRIMPOLY
            std::cout << "Coefficient of polynomial is primitive root.\n" << std::endl;
            #endif
            ...
            if (!linear_factor( f, n, p ))
            {
                ...
                #ifdef DEBUG_PP_PRIMPOLY
                std::cout << "Free of linear factors.\n" << std::endl;
                #endif
                ...
                if (!has_multi_irred_factors( power_table, n, p ))
                {
                    ...
                    #ifdef DEBUG_PP_PRIMPOLY
                    std::cout <<  "Has one unique irreducible factor.\n"  << std::endl;
                    #endif
                    ...
                    if (order_r( power_table, n, p, r, &a ))
                    {
                        ...
                        #ifdef DEBUG_PP_PRIMPOLY
                        std::cout << "Passes the order r test.\n" << std::endl;
                        #endif
                         ...
                         if (const_coeff_test( f, n, p, a ))
                         {
                             ...
                             #ifdef DEBUG_PP_PRIMPOLY
                             std::cout << "Passes the constant coefficient test.\n" << std::endl;
                             #endif
                             ...
                             if (order_m( power_table, n, p, r, primes, prime_count ))
                             {
                                 ...
                                 #ifdef DEBUG_PP_PRIMPOLY
                                 std::cout << "Passes the order m tests.\n" << std::endl;
                                 #endif

                                 if (listAllPrimitivePolynomials)
                                 {
                                     std::cout
                                         << "\n\nPrimitive polynomial " << ++prim_poly_count
                                         << " of " << num_prim_poly
                                         << " modulo " << p
                                         << " of degree " << n
                                         << "\n"
    ...
    */
    ...
    else
    {
        std::cout <<
            "Internal error:  \n"
            "Tested all possible polynomials " << max_num_poly << ", but failed\n"
    ...
    if (printStatistics)
    {
        std::cout
            << "+--------- Statistics -----------------------------------------------------------------\n"
            << "|\n"
            << "| Total num. degree: " << n << " polynomials mod " << p << " :  " << max_num_poly << "\n"
            << "| Actually tested:                     " << num_poly << "\n"
            << "| Const. coeff. was primitive root:    " << num_const_coeff_prim_root  << "\n"
            << "| Free of linear factors:              " << num_free_of_linear_factors  << "\n"
            << "| Irreducible or irred. to power:      " << num_irred_to_power  << "\n"
            << "| Had order r (x^r = integer):         " << num_order_r  << "\n"
            << "| Passed const. coeff. test:           " << num_passing_const_coeff_test  << "\n"
            << "| Had order m (x^m != integer):        " << num_order_m << "\n"
            << "|\n"
            << "+--------------------------------------------------------------------------------------\n"
            << std::endl;
        ...
        */
        ...
        if (selfCheck && !listAllPrimitivePolynomials)
        {
            ...
            if (maximal_order( f, n, p ))
                std::cout << "    -Polynomial is confirmed to be primitive.\n\n" << std::endl;
